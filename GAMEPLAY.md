...
- **Déplacement borné — modèle retenu (précision technique actée)** : un cercle de déplacement (centre + rayon), pas un budget cumulatif de distance parcourue. **Jusqu'à 2 phases de déplacement par tour**, avec une action facultative entre les deux (attaquer, lancer une synthèse, autre — le jeu des actions reste à définir). Après cette action, un nouveau cercle est recentré sur la position au moment de l'action, avec pour rayon le déplacement restant — permet de bouger, agir, puis se replacer (couverture, proximité d'une source d'élément) sans pénaliser les allers-retours. Détail d'implémentation dans `PROGRESS.md`.
...
- **Idée notée — rayon de détection d'engagement par ennemi** : le rayon de la zone qui déclenche l'entrée en combat (actuellement une `Area3D` de taille fixe sur chaque ennemi) devrait varier selon le type d'ennemi — probablement porté par une future `Resource` par ennemi, à rattacher aux autres stats d'ennemi déjà évoquées (zone de patrouille, agressivité). Non implémenté, non tranché sur la structure exacte.
- **Idée notée — rejoindre un combat à distance via notification** : second chemin d'entrée en combat, en plus du contact physique existant (`PlayerDetector`) — un joueur trop loin pour courir jusqu'au combat recevrait une notification à valider, entraînant une téléportation instantanée vers le combat. Différence clé avec une téléportation classique : **pas de retour à la position d'origine après le combat** (simplifie l'implémentation, pas de position à sauvegarder/restaurer). Questions non tranchées : qui reçoit la notification (tous les joueurs en exploration à portée ? seulement ceux dans un certain rayon du combat ?), où le joueur téléporté atterrit-il (autour du point de contact initial ? en bord de la zone de détection ?). Non implémenté — à creuser dans une session dédiée, distincte du déplacement borné.
